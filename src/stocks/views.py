...
from .forms import StockListForm, StockListUpdateForm, StockListItemForm, StockListSelectForm
...
class StockListSelectForm(forms.Form):
    stocklist = forms.ModelChoiceField(queryset=None, label="Vorratsliste auswählen")

    def __init__(self, *args, **kwargs):
        user = kwargs.pop('user')
        super().__init__(*args, **kwargs)
        self.fields['stocklist'].queryset = StockList.objects.filter(owner=user)

def transfer_shoppinglist_to_stock(request, pk):
    shoppinglist = ShoppingList.objects.get(pk=pk)
    if request.method == 'POST':
        form = StockListSelectForm(request.POST, user=request.user)
        if form.is_valid():
            stocklist = form.cleaned_data['stocklist']
        ...
        form = StockListSelectForm(user=request.user)
    return render(request, 'shoppinglist_item/transfer_to_stock.html', {
        'form': form,
